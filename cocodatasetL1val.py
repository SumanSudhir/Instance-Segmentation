...
def main():
    ...
    target_class_string_to_class_id = {class_string:i+1 for i, class_string in enumerate(sorted(target_class_strings))}

    parent_class_strings = list(set(class_string_to_parent.values()))
    layer0_independent_class_strings = [class_string for class_string in layer0_class_strings if class_string not in parent_class_strings]
    ...
    output_dir = data_dir.joinpath("cocoL1val")
    ...
    for i, img_id in enumerate(tqdm(target_img_ids)):
        ...
        for m in img_id_to_meta[img_id]:
            ...
            # for layer1: fill non-target bbox with gray; this class has its child in layer 1
            else:
    ...
    categories = [{"supercategory": "object", "id": class_id, "name": class_string} for class_string, class_id in target_class_string_to_class_id.items()]
